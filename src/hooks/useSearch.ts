...
export const useSearch = ({ 
  ...
  const availableFilters = useMemo(() => {
    const categories = Array.from(new Set(
      notes.map(note => note.category || 'personal')
    )).sort()
    ...
    const colors = Array.from(new Set(
      notes.map(note => note.color || '#f8fafc')
    ))
  ...
  const suggestions = useMemo(() => {
    ...
    const matchingHistory = searchHistory.filter(historyQuery =>
  ...
  const toggleFilterValue = useCallback(<K extends keyof SearchFilters>(
    key: K,
    value: any
  ) => {
    setFilters(prev => {
  ...
  const executeSearch = useCallback((searchQuery: string, searchFilters?: Partial<SearchFilters>) => {
